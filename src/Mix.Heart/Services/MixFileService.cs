...
namespace Mix.Heart.Services {
  public class MixFileService : SingletonService<MixFileService> {
    ...
    /// <summary>
    /// Prevents a default instance of the <see cref="MixFileRepository"/> class
    /// from being created.
    /// </summary>
    public MixFileService() { CurrentDirectory = Environment.CurrentDirectory; }

#region Read Files

    public FileModel GetFile(string name, string ext, string FileFolder,
                             bool isCreate = false,
      ...
      if (fileinfo.Exists) {
        try {
          using (var stream = File.Open(fullPath, FileMode.Open,
            ...
            using (StreamReader s = new StreamReader(stream)) {
              result =
                  new FileModel() { FileFolder = FileFolder, Filename = name,
                                    Extension = ext, Content = s.ReadToEnd() };
            }
          }
        } catch {
          // File invalid
        ...
        fileinfo.Create();
        result = new FileModel() { FileFolder = FileFolder, Filename = name,
                                   Extension = ext, Content = defaultContent };
        SaveFile(result);
      }
      ...
      if (Directory.Exists(folder)) {
        ...
        foreach (var file in Files.OrderByDescending(f => f.CreationTimeUtc)) {
          result.Add(new FileModel() {
            FolderName = folderName, FileFolder = folder,

            Filename = file.Name.Substring(0, file.Name.LastIndexOf('.') >= 0
...
#region Folder
    ...
    public bool CopyFolder(string srcPath, string desPath) {
      if (srcPath.ToLower() != desPath.ToLower() && Directory.Exists(srcPath)) {
        ...
        foreach (string dirPath in Directory.GetDirectories(
                     srcPath, "*", SearchOption.AllDirectories)) {
    ...
    public string ZipFolder(string tmpPath, string outputPath,
                            string fileName) {
      try {
        // string tmpPath =
        // $"wwwroot/Exports/temp/{fileName}-{DateTime.UtcNow.ToShortDateString()}";
