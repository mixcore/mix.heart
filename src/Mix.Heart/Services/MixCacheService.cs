...
namespace Mix.Heart.Services {
  public class MixCacheService {
    private readonly MixHeartConfigurationModel _configs =
        new MixHeartConfigurationModel();
    private readonly EntityRepository<MixCacheDbContext, MixCache, string>
        _repository;
    ...
    SaveDatabaseAsync<T>(string key, T value, Type dataType, string filename) {
      if (value != null) {
        ...
        var cache =
            new MixCache() { Id = $"{dataType.FullName}_{key}_{filename}",
