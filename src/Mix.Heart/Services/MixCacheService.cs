...
namespace Mix.Heart.Services {
  public class MixCacheService {
    ...
    public MixCacheService(
        IConfiguration configuration, MixFileService fileService,
        EntityRepository<MixCacheDbContext, MixCache, string> repository) {
      _configuration = configuration;
      _fileService = fileService;
      _configs = JObject.Parse(_configuration.GetSection("MixHeart").Value)
    ...
    private bool SaveJson<T>(string key, T value, string folder) {
      try {
        ...
        var cacheFile =
            new FileModel() { Filename = key.ToLower(), Extension = ".json",
    ...
    private async Task<bool> SaveDatabaseAsync<T>(string key, T value,
                                                  string folder) {
      if (value != null) {
        var jobj = JObject.FromObject(value);

        var cache = new MixCache() { Id = $"{folder}/{key}",
    ...
    public async Task RemoveCacheAsync(string folder) {
      switch (_configs.CacheMode) {
      case MixCacheMode.DATABASE:
        await _repository.DeleteManyAsync(
            m => EF.Functions.Like(m.Id, $"%{folder}%"));
        break; case MixCacheMode.JSON : default
            : _fileService.DeleteFolder($"{_configs.CacheFolder}/{folder}");
            break;
      }
    }
