...
#region Get
  ...
  public Task<T> GetAsync<T>(string key, string cacheFolder, string filename,
                             CancellationToken cancellationToken = default)
      where T : class {
    return _cache.GetFromCache<T>($"{cacheFolder}_{key}_{filename}",
                                  cancellationToken);
  }
#endregion
...
#region Set
  ...
  public Task SetAsync<T>(string key, T value, string cacheFolder,
                          ...
                          CancellationToken cancellationToken = default)
      where T : class {
    return _cache.SetCache($"{cacheFolder}_{key}_{filename}", value,
                           cancellationToken);
  }
#endregion
