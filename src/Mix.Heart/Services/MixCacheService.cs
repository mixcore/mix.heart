...
namespace Mix.Heart.Services {
  public class MixCacheService {
    ...
    private readonly EntityRepository<MixCacheDbContext, MixCache, Guid>
        _repository;
    ...
    private bool SaveJson<T>(string key, T value, Type dataType,
      ...
      try {
        ...
        var cacheFile = new FileModel() {
          Filename = filename.ToLower(), Extension = ".json",
          FileFolder =
              $"{_configs.CacheFolder}/{dataType.FullName}/{key.ToLower()}",
      ...
      try {
        ...
        default:
          return Task.FromResult(
              GetFromJson<T>(key, dataType.FullName, filename));
        }
      } catch (Exception ex) {
    ...
    private T GetFromJson<T>(string key, string folder, string filename) {
      string filePath =
          $"{_configs.CacheFolder}/{folder}/{key}/{filename}.json";
      if (File.Exists(filePath)) {
        using (StreamReader file = File.OpenText(filePath)) using (
    ...
    GetFromDatabaseAsync<T>(string key, string folder, string filename,
                            CancellationToken cancellationToken = default) {
      try {
    ...
    SetAsync<T>(string key, T value, Type dataType, string filename,
                CancellationToken cancellationToken = default) {
      if (value != null) {
        switch (_configs.CacheMode) {
        case MixCacheMode.DATABASE:
    ...
    SaveDatabaseAsync<T>(string key, T value, Type dataType, string filename,
                         CancellationToken cancellationToken = default) {
      if (value != null) {
        var jobj = JObject.FromObject(value);

        var cache =
            new MixCache() { Id = Guid.NewGuid(),
    ...
    public async
      ...
      case MixCacheMode.DATABASE:
        ...
        break; case MixCacheMode.JSON : default
    ...
    public async
        Task RemoveCacheAsync(object key, Type dataType,
                              CancellationToken cancellationToken = default) {
      switch (_configs.CacheMode) {
      case MixCacheMode.DATABASE:
        await _repository.DeleteAsync(
            m => m.Keyword == $"{dataType.FullName}_{key}", cancellationToken);
        break; case MixCacheMode.JSON : default
