...
namespace Mix.Heart.ViewModel {
  public class CommandViewModelBase<TDbContext, TEntity, TPrimaryKey>
      : ViewModelBase<TDbContext, TEntity, TPrimaryKey> where TPrimaryKey
      : IComparable where TEntity : class,
                                    IEntity<TPrimaryKey> where TDbContext
      : DbContext {
...
#region Async
    ...
    public virtual async Task<TEntity> SaveHandlerAsync() {
      ...
      var config = new MapperConfiguration(
          cfg => cfg.CreateMap(typeof(TEntity), GetType()).ReverseMap());
