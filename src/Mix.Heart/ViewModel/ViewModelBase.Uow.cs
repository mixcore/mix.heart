...
namespace Mix.Heart.ViewModel {
  public abstract
      ...
      if (_unitOfWorkInfo != null) {
        ...
        if (_unitOfWorkInfo.ActiveTransaction == null) {
          ...
          _unitOfWorkInfo.SetTransaction(
              _unitOfWorkInfo.ActiveDbContext.Database
                  .CurrentTransaction
              ?? _unitOfWorkInfo.ActiveDbContext.Database.BeginTransaction());
        }
        _repository ?? =
            new CommandRepository<TDbContext, TEntity, TPrimaryKey>(
                _unitOfWorkInfo);
    ...
    private void InitRootUow() {
      ...
      _repository ?? = new CommandRepository<TDbContext, TEntity, TPrimaryKey>(
          _unitOfWorkInfo);
      _repository.SetUowInfo(_unitOfWorkInfo);
    }
    ...
    protected virtual void CloseUow() { _unitOfWorkInfo.Close(); }
