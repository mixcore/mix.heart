...
#region Abstracts
    ...
    GetRepository(UnitOfWorkInfo uowInfo) {
      return new Repository<TDbContext, TEntity, TPrimaryKey, TView>(uowInfo);
    }

    // use for public
    public static Repository<TDbContext, TEntity, TPrimaryKey, TView>
    GetRootRepository(TDbContext context) {
      return new Repository<TDbContext, TEntity, TPrimaryKey, TView>(context);
    }

    public virtual async Task Validate() {
      var validateContext =
          new System.ComponentModel.DataAnnotations.ValidationContext(
              this, serviceProvider: null, items: null);
    ...
    public void SetConsumer(IMixMediator consumer) { _consumer = consumer; }
    ...
    public virtual void ParseView<TSource>(TSource sourceObject)
      ...
      var config = new MapperConfiguration(
          cfg => cfg.CreateMap(typeof(TSource), GetType()));
      var mapper = new Mapper(config);
      mapper.Map(sourceObject, this);
    }

    public Task PublishAsync(object sender, MixViewModelAction action,
                             bool isSucceed, Exception ex = null) {
      return _consumer != null
                 ? _consumer.ConsumeAsync(sender, action, isSucceed)
                 : Task.CompletedTask;
    }

    public virtual Task ConsumeAsync(object sender, MixViewModelAction action,
                                     bool isSucceed, Exception ex = null) {
    ...
    protected bool IsDefaultId(TPrimaryKey id) {
      return (id.GetType() == typeof(Guid) &&
             ...
             (id.GetType() == typeof(int) &&
              int.Parse(id.ToString()) == default);
    }
    ...
    protected void MapObject<TSource, TDestination>(TSource sourceObject,
                                                    TDestination destObject)
        where TSource : class
      ...
      var commonProperties =
          from sourceProperty in sourceProperties join destProperty in
              destProperties on new { sourceProperty.Name,
                                      sourceProperty.PropertyType } equals new {
                destProperty.Name, destProperty.PropertyType
              } select new { SourceProperty = sourceProperty,
                             DestProperty = destProperty };

      foreach (var property in commonProperties) {
        property.DestProperty.SetValue(
            destObject, property.SourceProperty.GetValue(sourceObject));
      }
    }
  }
}
