...
#region Properties
    ...
    public QueryRepository() { SelectedMembers = FilterSelectedFields(); }

    public QueryRepository(UnitOfWorkInfo unitOfWorkInfo)
        : base(unitOfWorkInfo) {
...
#region Entity Async
    ...
    public async Task<PagingResponseModel<TEntity>>
    GetPagingEntitiesAsync(Expression<Func<TEntity, bool>> predicate,
                           PagingModel paging,
...
#region Helper
    private string[] FilterSelectedFields() {
      ...
      return properties.Where(p => p.PropertyType.IsSerializable)
          .Select(p => p.Name)
          .ToArray();
    }

    protected async Task<PagingResponseModel<TEntity>>
    ToPagingEntityAsync(IQueryable<TEntity> source, PagingModel pagingData,
                        CancellationToken cancellationToken = default) {
      try {
