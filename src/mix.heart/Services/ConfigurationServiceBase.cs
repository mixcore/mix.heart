...
public class ConfigurationServiceBase<T> {
  ...
  public TValue GetConfig<TValue>(string name, TValue defaultValue = default) {
    if (RawSettings.TryGetValue(name, StringComparison.OrdinalIgnoreCase,
                                out var result)) {
  ...
  public TValue GetConfig<TValue>(string culture, string name,
  ...
  public TValue GetEnumConfig<TValue>(string name) {
    Enum.TryParse(typeof(TValue), RawSettings[name]?.Value<string>(), true,
                  out object result);
    return result != null ? (TValue)result : default;
  }

  public void SetConfig<TValue>(string name, TValue value) {
    RawSettings[RawSettings.Property(name, StringComparison.OrdinalIgnoreCase)
                    .Name] = value != null ? JToken.FromObject(value) : null;
  ...
  public virtual bool SaveSettings() {
    var settings = MixFileHelper.GetFileByFullName(
        $"{FilePath}{MixFileExtensions.Json}", true, "{}");
    if (settings != null) {
      settings.Content = string.IsNullOrEmpty(SectionName)
                             ? RawSettings.ToString(Formatting.None)
                             : ReflectionHelper
                                   .ParseObject(new JObject(
                                       new JProperty(SectionName, RawSettings)))
                                   .ToString(Formatting.None);
      if (_isEncrypt) {
        settings.Content =
            AesEncryptionHelper.EncryptString(settings.Content, AesKey);
      }
  ...
  protected virtual void LoadAppSettings() {
    var settings = MixFileHelper.GetFileByFullName(
        $"{FilePath}{MixFileExtensions.Json}", true, "{}");
    string content =
        string.IsNullOrWhiteSpace(settings.Content) ? "{}" : settings.Content;
