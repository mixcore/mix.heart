...
public class MixDatabaseCacheClient : IDitributedCacheClient {
  ...
  public async Task<T>
  GetFromCache<T>(string key, CancellationToken cancellationToken = default)
      where T : class {
    try {
      var cache = await _repository.GetFirstAsync(m => m.Keyword == key,
                                                  cancellationToken);
      if (cache != null) {
        try {
  ...
  public async Task SetCache<T>(string key, T value,
    ...
    if (value != null) {
      ...
      var cache = new MixCache() { Id = Guid.NewGuid(), Keyword = key,
