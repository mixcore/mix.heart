...
public class MixException : Exception {
  ...
  protected MixException(SerializationInfo info, StreamingContext context) {
    LogException(message: Message);
  }

  public MixException(MixErrorStatus status, params object[] messages)
      : base(string.Join('\n', messages)) {
    Status = status;
    Value = messages;
    LogException(status: status, message: Message);
  ...
  public MixException(MixErrorStatus status, Exception ex)
      : base(ex.InnerException?.Message ?? ex.Message) {
    Status = status;
    Value = ex.Data;
    LogException(ex);
  }
  ...
  private void LogException(Exception ex = null, MixErrorStatus? status = null,
                            string message = null) {
    Console.Error.Write(ex);

    // string fullPath =
    // $"{Environment.CurrentDirectory}/logs/{DateTime.Now:dd-MM-yyyy}"; if
    // (!string.IsNullOrEmpty(fullPath) && !Directory.Exists(fullPath))
    //{
    ...
    //        new JProperty("Details", ex == null ? null :
