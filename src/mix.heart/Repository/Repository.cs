...
#region Async
  ...
  UpdateAsync(TEntity entity, CancellationToken cancellationToken = default) {
    try {
      ...
      if (!await CheckIsExistsAsync(entity, cancellationToken))
        throw new MixException(Enums.MixErrorStatus.NotFound,
                               entity.Id.ToString());
      ...
      if (CacheService != null) {
        await CacheService.RemoveCacheAsync(entity.Id, CacheFolder,
                                            cancellationToken);
      }
    } catch (Exception ex) {
      await HandleExceptionAsync(ex);
  ...
  public virtual async
    ...
    try {
      BeginUow();
      var entity = await Context.Set<TEntity>().SingleOrDefaultAsync(
    ...
    try {
      ...
      if (CacheService != null) {
        await CacheService?.RemoveCacheAsync(
            $"{typeof(TEntity).FullName}_{entity.Id}*", CacheFolder,
            cancellationToken);
      }
    } catch (Exception ex) {
      await HandleExceptionAsync(ex);
